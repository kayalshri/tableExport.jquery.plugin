...
if("base64"===a.outputMode)return K(P);if("window"===a.outputMode){ja(!1,"data:text/"+("csv"===a.type?"csv":"plain")+";charset=utf-8,",P);return}M(P,a.fileName+"."+a.type,"text/"+("csv"===a.type?"csv":"plain"),"utf-8","","csv"===a.type&&a.csvUseBOM)}else if("sql"===a.type){p=0;D=[];var w="INSERT INTO "+a.sql.tableEnclosure+a.tableName+a.sql.tableEnclosure+" (";u=y(c(A));c(u).each(function(){E(this,"th,td",p,u.length,function(b,c,e){w+=a.sql.columnEnclosure+F(b,c,e)+a.sql.columnEnclosure+","});p++;
w=c.trim(w).substring(0,w.length-1)});w+=") VALUES ";r=B(c(A));c(r).each(function(){n="";E(this,"td,th",p,u.length+r.length,function(a,c,e){n+="'"+F(a,c,e)+"',"});3<n.length&&(w+="("+n,w=c.trim(w).substring(0,w.length-1),w+="),");p++});w=c.trim(w).substring(0,w.length-1);w+=";";if("string"===a.outputMode)return w;if("base64"===a.outputMode)return K(w);M(w,a.fileName+".sql","application/sql","utf-8","",!1)}else if("json"===a.type){var W=[];D=[];u=y(c(A));c(u).each(function(){var a=[];E(this,"th,td",
...
d.find("tr, th, td").filter(":hidden"),V=0<J.length);p=0;T=S(this);H="<Table>\r";e=b(y(d),"th,td",0);b(B(d),"td,th",e);H+="</Table>\r";sa.push(H)});h={};v={};for(var l,R,X=0,ca=G.length;X<ca;X++)l=G[X],R=h[l],R=h[l]=null==R?1:R+1,2===R&&(G[v[l]]=G[v[l]].substring(0,29)+"-1"),1<h[l]?G[X]=G[X].substring(0,29)+"-"+h[l]:v[l]=X;h='<?xml version="1.0" encoding="UTF-8"?>\r<?mso-application progid="Excel.Sheet"?>\r<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"\r xmlns:o="urn:schemas-microsoft-com:office:office"\r xmlns:x="urn:schemas-microsoft-com:office:excel"\r xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"\r xmlns:html="http://www.w3.org/TR/REC-html40">\r<DocumentProperties xmlns="urn:schemas-microsoft-com:office:office">\r  <Created>'+
...
1,C+=v;var B=0;for(l=n=0;l<A.length;++l)if(r=A[l],v=+O(r)||1,m=l+B,!ta(c(r),C,m+(m<n?n-m:0))){B+=v-1;for(m=0;m<f.length;++m){var w=f[m];w.s.c==n&&w.s.r<=h&&h<=w.e.r&&(n=w.e.c+1,m=-1)}(0<(u=+U(r))||1<v)&&f.push({s:{r:h,c:n},e:{r:h+(u||1)-1,c:n+v-1}});var E={type:""},G=c(r).attr("data-tableexport-cellformat");m=F(r,k,l+B,E);w={t:"s",v:m};var D="";if(""!==G){var x=parseInt(c(r).attr("data-tableexport-xlsxformatid")||0);0===x&&"function"===typeof a.mso.xslx.formatId.numbers&&(x=a.mso.xslx.formatId.numbers(c(r),
