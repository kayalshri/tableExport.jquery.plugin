...
e.toLowerCase()}function ba(b,a){(b=/^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/.exec(b))&&(a=[parseInt(b[1]),parseInt(b[2]),parseInt(b[3])]);return a}function ta(a){var b=K(a,"text-align"),e=K(a,"font-weight"),c=K(a,"font-style"),h="";"start"===b&&(b="rtl"===K(a,"direction")?"right":"left");700<=e&&(h="bold");"italic"===c&&(h+=c);""===h&&(h="normal");b={style:{align:b,bcolor:ba(K(a,"background-color"),[255,255,255]),color:ba(K(a,"color"),[0,0,0]),fstyle:h},colspan:S(a),rowspan:T(a)};null!==a&&
(a=a.getBoundingClientRect(),b.rect={width:a.width,height:a.height});return b}function S(a){var b=c(a).data("tableexport-colspan");"undefined"===typeof b&&c(a).is("[colspan]")&&(b=c(a).attr("colspan"));return parseInt(b)||0}function T(a){var b=c(a).data("tableexport-rowspan");"undefined"===typeof b&&c(a).is("[rowspan]")&&(b=c(a).attr("rowspan"));return parseInt(b)||0}function K(a,d){try{return window.getComputedStyle?(d=d.replace(/([a-z])([A-Z])/,Ea),window.getComputedStyle(a,null).getPropertyValue(d)):
